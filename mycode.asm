...
Loop1:    
    ...
    PRINT "Cipher Text: "
    PRINTN
    CALL PRINT_ARRAY             ;Got from Google 
    PRINTN
ret
...
KeySchedule PROC
    ...
    MOV newkey[8],AL     
            ...
            ;#procedure_explanation: substitution of bytes with Sbox
...
KeySchedule ENDP   
 ...
 ;ENCYRPTION
 ; First Step: SubBytes(MACRO);substitution of bytes with sbox            
...
Step1 PROC
   ...
   Step1L1:                   ; outer loop         
     ...
     Step1L2:                 ; inner loop     
       ...
       MOV  AH,sbox[DI]             ; get the byte from sub matrix
       MOV  a[SI],AH                 ; update the array with the substitution  
...
;Second Step: ShiftRows(MACRO)   
 ;#procedure_explanation: the bytes in the last three rows of the State are cyclically
